...
def process_snope():
    ...
    for i in range(5):
        ...
        name_pickle = f'/Users/namle/Desktop/GET/saved_model/Snopes/snope_{i}'
        ...
        output_name = f'embed_probagation/output_snope_train_{i}.pt'
        logit_name = f'embed_probagation/logit_snope_train_{i}.pt'
        torch.save(outputs, output_name)
        torch.save(logits, logit_name)

        logits,outputs = get_energy(predict_interactions, fit_model)
        output_name = f'embed_probagation/output_snope_test_{i}.pt'
        logit_name = f'embed_probagation/logit_snope_test_{i}.pt'
        torch.save(outputs, output_name)
        torch.save(logits, logit_name)
        if i ==4:
            ...
            output_name = f'embed_probagation/output_snope_valid.pt'
            logit_name = f'embed_probagation/logit_snope_valid.pt'
            torch.save(outputs, output_name)
            torch.save(logits, logit_name)
